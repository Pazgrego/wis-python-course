...
_HINT_NOT_DILUTION = (
    "Hint: C2 >= C1 — not a dilution.\n"
    "The target is at or above stock concentration (you would need a stronger stock to reach C2)."
)
...
def on_calculate():
    ...
    if c2 >= c1:
        label_hint.config(text=_HINT_NOT_DILUTION, fg="#8B4513")
    else:
...
label_hint = tk.Label(root, text="", justify="left", wraplength=320, font=("TkDefaultFont", 9))
label_hint.grid(row=5, column=0, columnspan=2, **padding)
